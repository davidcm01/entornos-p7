...
ArrayList<Equipo> equipos = bdcontroller.todosEquipos();
Jugador jugador = bdcontroller.dameJugador(Integer.parseInt(request.getParameter("cod_jugador")));
for(int i =0;i<equipos.size();i++){
	if(equipos.get(i).getCod_equipo()==jugador.getCod_equipo()){
		equipos.remove(i);
	}
}
					...
					<div class="container">
						<div id="content">
							...
							<!-- Content -->
								...
								<form action="operaciones_alu.jsp?tipo=modificarjugador" method="post">
									<div class="row gtr-50">
										<div class="">
											<input type="text" value="<%=jugador.getNombre() %>" required name="nombre" id="nombre" placeholder="Nombre del jugador" style="width:450px"; maxlength="20"/>
										</div>
										<div class="">
											<input type="text" value="<%=jugador.getPierna() %>" required name="pierna" id="pierna" placeholder="Pierna buena del jugador" maxlength="9"/>
										</div>
										<div class="">
											
											<select  name="codEquipo" id="cod_equipo" is ="ms-dropdown" required style="width:450px" data-child-height="415">
											<option value="<%=jugador.getCod_equipo()%>"data-image="images/equipos/<%=jugador.getCod_equipo()%>.png"><%=bdcontroller.dameEquipo(jugador.getCod_equipo()).getNombre()%></option>
											<%for(int i =0;i<equipos.size();i++){	
										...
										<div class="">
											<input type="text" value="<%=jugador.getPais() %>" required name="pais" id="pais" placeholder="País del jugador " maxlength="20"/>
										</div>
										<div class="">
											<input type="number" value="<%=jugador.getAltura() %>" required name="altura" id="altura" placeholder="Altura del jugador" maxlength="4"/>
										</div>
									...
									<div style="padding-top:22px;">
											<ul class="actions">
												<li><input type="submit" class="button alt" value="Actualizar" /></li>
											</ul>
									</div>
